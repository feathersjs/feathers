...
var SocketIoProvider = Proto.extend({
	...
	start: function (server, httpServer) {
		var io = this._io(httpServer);
		...
		io.sockets.on('connection', function (socket) {
			_.each(services, function (service, path) {
				_.each(server.config.methods, function(method) {
		...
		console.log('Feathers Web Socket provider initialized');
