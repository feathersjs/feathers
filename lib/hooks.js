  ...
  methods.forEach(method => {
    ...
    mixin[method] = function () {
      ...
      const pickArgsHook = context => {
        const argsList = Object.assign({}, argumentsOrders, serviceMethods);
        const methodArgsList = argsList[method];

        const argsObject = args.reduce(
          (result, value, index) => Object.assign(result, { [methodArgsList[index]]: value }),
          { params: {} }
        );
      ...
      const beforeHooks = [ pickArgsHook, validateHook, ...getHooks(app, service, 'before', method) ];
