...
const ACTIVE_HOOKS = typeof Symbol !== 'undefined'
...
const hookMixin = exports.hookMixin = function hookMixin (service) {
  ...
  service.methods = Object.keys(service)
    .filter(key => typeof service[key] === 'function' && service[key][ACTIVE_HOOKS])
    .reduce((result, methodName) => {
      result[methodName] = service[methodName][ACTIVE_HOOKS];
      return result;
    }, service.methods || {});
  ...
  methodNames.forEach(method => {
    ...
    mixin[method] = function () {
      ...
      const validateHook = context => {
        validateArguments(methods, method, args);
      ...
      const pickArgsHook = context => {
        const argsObject = args.reduce(
          (result, value, index) => {
            result[methods[method][index]] = value;
            return result;
          },
          { params: {} }
        );
      ...
      const beforeHooks = [ validateHook, pickArgsHook, ...getHooks(app, service, 'before', method) ];
...
module.exports.activeHooks = function activeHooks (args) {
  return fn => {
