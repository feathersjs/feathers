...
function getEventNamedArgs(args, event) {
  switch (event) {
    case 'update':
    case 'patch':
      return { id: args[0], data: args[1], params: args[2] };
    case 'create':
      return { data: args[0], params: args[1] };
    case 'remove':
      return { id: args[0], params: args[1] };
    default:
      return {};
  }
}
      ...
      if (typeof self[method] === 'function' && !alreadyEmits) {
        ...
        emitter.on(eventName, function (results, args) {
          if (!results[0]) { // callback without error
            var namedArgs = getEventNamedArgs(args, method);
            self.emit(event, results[1], namedArgs);
          } else {
