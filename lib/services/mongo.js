...
var MongoService = Proto.extend({
  ...
  _connect: function(options){
    this.host = process.env.MONGO_HOST || options.host || 'localhost';
    this.port = process.env.MONGO_PORT || options.port || 27017;
    this.database = process.env.MONGO_DB || options.database || 'feathers';
  ...
  destroy: function (id, params, cb) {
	...
	setup: function(server, location) {
