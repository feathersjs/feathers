...
With a query schema, the adapter does not re-run its `$` operator allowlist. The schema is the full allowlist. Use `querySyntax` / query helpers and `additionalProperties: false` so unknown operators cannot pass through. See [validateQuery](../schema/validators.md#validatequery) for details.
