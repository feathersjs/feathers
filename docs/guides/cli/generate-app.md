...
Depending on which option you Select, Feathers will install it alongside the JWT authentication strategy package. After authentication with the chosen method, Feathers gives you a JWT which can be used to authenticate with the API.
...
If you have a local database server running, you're ready to start your application. Run the following command to start a development server that automatically restarts after files are changed:
