...
The `authenticate` hook will use `params.authentication` of the service method call and run [authenticationService.authenticate()](./service#authenticate-data-params-strategies).
...
For example, a successful [JWT strategy](./jwt) authentication will set:
...
In the following hooks and for the service method call. It can be used as a `before` or `around` [hook](../hooks).
