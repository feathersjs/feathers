...
When the `sse` option is set on the [fetch client](./rest), the client will automatically establish an SSE connection during [app.setup()](../application#setupserver) to receive real-time events from the server via [channels](../channels).
