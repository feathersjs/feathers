...
::note[Note]
For additional information also see the [TypeBox documentation](https://github.com/sinclairzx81/typebox/blob/master/readme#contents).
::

## Usage

The module exports all of TypeBox functionality with additional support for [query schemas](#query-schemas) and [validators](#validators). The following is an example for defining the message schema [from the guide](../../guides/basics/schemas#handling-messages) using TypeBox:
...
`querySyntax(definition, extensions, options)` returns a schema to validate the [Feathers query syntax](../databases/querying) for all properties in a TypeBox definition.
...
Additional special query properties [that are not already included in the query syntax](../databases/querying) like `$ilike` can be added like this:
...
`queryProperty(definition)` returns a schema for the [Feathers query syntax](../databases/querying) for a single property.

## Validators

The following functions are available to get a [validator function](./validators) from a TypeBox schema.

::note[note]
See the [validators](./validators) chapter for more information on validators and validator functions.
::
