...
As an alternative to [TypeBox](./typebox), `@feathersjs/schema` also provides the ability to define plain JSON schemas as objects. It uses [json-schema-to-ts](https://github.com/thomasaribart/json-schema-to-ts) to turn those schemas into TypeScript types.

::note[Need JSON Schema help?]
...
Schema ships with a few helpers to automatically create schemas that comply with the [Feathers query syntax](../databases/querying) (like `$gt`, `$ne` etc.):
...
The following functions are available to get a [validator function](./validators) from a JSON schema definition.
