...
describe('@feathersjs/authentication-oauth/express', () => {
  ...
  it('oauth/test', async () => {
    try {
      await axios.get<any>('http://localhost:9876/oauth/test?feathers_token=testing', {
        maxRedirects: 0
      })
    } catch (error: any) {
  ...
  it('oauth/test with query', async () => {
    try {
      await axios.get<any>('http://localhost:9876/oauth/test?other=test', {
        maxRedirects: 0
      })
    } catch (error: any) {
      assert.equal(error.response.status, 302)
  ...
  it('oauth/test/authenticate', async () => {
    const { data } = await axios.get<any>(
      'http://localhost:9876/oauth/test/authenticate?profile[sub]=expressTest'
    )
