...
const js = (ctx: AppGeneratorContext) => (ctx.framework === 'express' ? jsExpressApp(ctx) : jsKoaApp(ctx))
...
const ts = (ctx: AppGeneratorContext) => (ctx.framework === 'express' ? tsExpressApp(ctx) : tsKoaApp(ctx))
