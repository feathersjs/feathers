...
import { NextFunction } from "https://deno.land/x/hooks@v0.7.5/src/index.ts";
  ...
  return next().then(() => {
    ...
    if (typeof context.event === "string" && !events?.includes(context.event)) {
      ...
      results.forEach((element) => {
        return (context as any).self.emit(context.event, element, context);
      });
    }
  });
}
...
export function eventMixin<A>(service: FeathersService<A>) {
  const isEmitter =
    typeof service.on === "function" && typeof service.emit === "function";
