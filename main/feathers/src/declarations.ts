...
export type Service<T = any, D = Partial<T>, P = Params> = ServiceMethods<
  T,
  D,
  P
> &
...
export type ServiceInterface<T = any, D = Partial<T>, P = Params> = Partial<
 ...
 */
export type CustomMethod<T = any, R = T, P extends Params = Params> = (
  data: T,
  params?: P
) => Promise<R>;
...
export type ServiceGenericType<S> = S extends ServiceInterface<infer T>
  ? T
  : any;
export type ServiceGenericData<S> = S extends ServiceInterface<
  infer _T,
  infer D
>
  ? D
  : any;
export type ServiceGenericParams<S> = S extends ServiceInterface<
  infer _T,
  infer _D,
  infer P
>
...
export interface HookContext<A = Application, S = any>
...
export interface ApplicationHookContext<A = Application>
...
export type ApplicationHookOptions<A> =
  | HookOptions<A, any>
  | ApplicationHookMap<A>;
