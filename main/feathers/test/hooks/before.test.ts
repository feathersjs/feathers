...
import { feathers, Params, ServiceInterface } from "../../mod.ts";
...
describe("`before` hooks", () => {
  it(".before hooks can return a promise", async () => {
    ...
    const app = feathers<{ dummy: DummyService }>().use("dummy", {
      async get(id: any, params: DummyParams) {
        assert(params.ran, "Ran through promise hook");
    ...
    await assertRejects(() => service.remove(10), "This did not work");
  });
  ...
  it("gets mixed into a service and modifies data", async () => {
    ...
    service.hooks({
      before: {
        create(context) {
          assertStrictEquals(context.type, "before");
    ...
    someService.hooks({
      before: {
        create(context) {
          ...
          assertStrictEquals(context.data!.appPresent, true);
  ...
  it("calling back with no arguments uses the old ones", async () => {
    ...
    const dummyService = {
      async remove(id: any, params: any) {
        assertStrictEquals(id, 1, "Got id");
        assertEquals(params, { my: "param" });
  ...
  it(".before hooks run in the correct order (#13)", async () => {
    ...
    const app = feathers<{ dummy: DummyService }>().use("dummy", {
      async get(id: any, params: any) {
        assertEquals(params.items, ["first", "second", "third"]);
  ...
  it("before all hooks (#11)", async () => {
    ...
    const app = feathers<{ dummy: DummyService }>().use("dummy", {
      async get(id: any, params: any) {
        assert(params.beforeAllObject);
        assert(params.beforeAllMethodArray);
