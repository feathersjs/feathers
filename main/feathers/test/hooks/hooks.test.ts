...
describe("hooks basics", () => {
  ...
  it("works with services that return a promise (feathers-hooks#28)", async () => {
    ...
    const app = feathers<{
      dummy: ServiceInterface<any, any, DummyParams>;
    }>().use("dummy", {
      async get(id: any, params: any) {
        return { id, user: params.user };
      },
    ...
    assertEquals(data, { id: 10, user: "David", after: true });
  });
  ...
  it("has context.app, context.service and context.path", async () => {
    ...
    service.hooks({
      before(context) {
        assertStrictEquals(this, service);
        assertStrictEquals(context.service, service);
        assertStrictEquals(context.app, app);
        assertStrictEquals(context.path, "dummy");
      },
    });
  ...
  describe("returns the context when passing it as last parameter", () => {
    it("on normal method call", async () => {
      ...
      assertStrictEquals(returnedContext.service, app.service("dummy"));
      assertStrictEquals(returnedContext.type, null);
      assertStrictEquals(returnedContext.path, "dummy");
      assertEquals(returnedContext.result, {
        id: 10,
        params: {},
      });
    });

    it.ignore("on error", async () => {
      const app = feathers().use("/dummy", {
        get() {
      ...
      const err: any = await assertRejects(() => service.get(10, {}, context));
    ...
    it("still swallows error if context.result is set", async () => {
      ...
      assert(returnedContext.error);
      assertEquals(returnedContext.result, result);
    });
  });
    ...
    service.hooks({
      before: {
        get: [
          ...
          (context) => {
            assertStrictEquals(context.value, "something");
          },
        ],
      },
      after: {
