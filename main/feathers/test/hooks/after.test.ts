...
import { feathers, Id } from "../../mod.ts";
...
describe("`after` hooks", () => {
  it(".after hooks can return a promise", async () => {
    ...
    await assertRejects(() => service.find({}), "You can not see this");
  });
  ...
  it("gets mixed into a service and modifies data", async () => {
    ...
    service.hooks({
      after: {
        create(context) {
          assertStrictEquals(context.type, "after");
    ...
    assertEquals({ my: "data", some: "thing" }, data, "Got modified data");
  });
  ...
  it("also makes the app available at hook.app", async () => {
    ...
    service.hooks({
      after: {
        create(context) {
          ...
          assertStrictEquals(context.result.appPresent, true);
    ...
    assertEquals(
      { my: "data", appPresent: true },
      data,
      "The app was present in the hook."
    );
  });
  ...
  it("does not run after hook when there is an error", async () => {
    ...
    await assertRejects(() => service.remove(1, {}), "Error removing item");
  });
  ...
  it(".after hooks run in the correct order (#13)", async () => {
    ...
    assertEquals(data.items, ["first", "second", "third"]);
  });
  ...
  it("after all hooks (#11)", async () => {
    ...
    assert(data.afterAllObject);
    assert(data.afterAllMethodArray);
