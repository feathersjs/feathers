...
import { JSONSchema } from "https://deno.land/x/json_schema_to_ts@v2.5.5/index.js";
import { Ajv, JSONSchemaDefinition, Validator } from "./schema.ts";
 ...
 */
export const querySyntax = <T extends { [key: string]: any }>(
  definition: T,
) => ({
  $limit: {
    type: "number",
  ...
  $sort: {
    type: "object",
    properties: Object.keys(definition).reduce(
      (res, key) => {
        const result = res as any;
      ...
      {} as {
        [K in keyof T]: { readonly type: "number"; readonly enum: [1, -1] };
      },
    ),
  },
  $select: {
