import { HookContext, NextFunction } from "../../../feathers/mod.ts";
import { BadRequest } from "../../../errors/mod.ts";
import { Schema, Validator } from "../schema.ts";
import { DataValidatorMap } from "../json-schema.ts";

export const validateQuery = <H extends HookContext>(
  schema: Schema<any> | Validator,
) => {
  const validator: Validator = typeof schema === "function"
    ? schema
...
export const validateData = <H extends HookContext>(
  schema: Schema<any> | DataValidatorMap,
) => {
  return async (context: H, next?: NextFunction) => {
    const data = context.data;
    const validator = typeof (schema as Schema<any>).validate === "function"
      ...
      try {
        if (Array.isArray(data)) {
          context.data = await Promise.all(
            data.map((current) => validator(current)),
          );
        } else {
