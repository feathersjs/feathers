...
import { compose } from "https://deno.land/x/hooks@v0.7.5/src/index.ts";
import { Resolver, ResolverStatus } from "../resolver.ts";
...
export type ResolverSetting<H extends HookContext> =
  | Resolver<any, H>
  | Resolver<any, H>[];
...
export const resolveData =
  ...
  async (context: H, next?: NextFunction) => {
    if (
      context.method === "create" || context.method === "patch" ||
      ...
      if (Array.isArray(data)) {
        context.data = await Promise.all(
          data.map((current) => runResolvers(resolvers, current, ctx, status)),
        );
      } else {
