...
export class Resolver<T, C> {
  ...
  async resolve<D>(
    ...
    const data = schema && validate === "before"
    ...
    await Promise.all(
      propertyList.map(async (name) => {
        ...
        if (resolvers[name]) {
          try {
            const resolved = await this.resolveProperty(
              name,
              data,
              context,
              status,
    ...
    if (hasErrors) {
      const propertyName = status?.properties
        ? ` ${status.properties.join(".")}`
        : "";

      throw new BadRequest(
        "Error resolving data" + (propertyName ? ` ${propertyName}` : ""),
        errors,
      );
    }
