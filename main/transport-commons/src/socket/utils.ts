...
import { BadRequest, MethodNotAllowed, NotFound } from "../../../errors/mod.ts";
import { createDebug } from "../../../commons/mod.ts";
import { lodash } from "https://deno.land/x/deno_ts_lodash@0.0.1/mod.ts";
import { CombinedChannel } from "../channels/channel/combined.ts";
...
export function getDispatcher(
  ...
  return function (
    ...
    channel.connections.forEach((connection) => {
      ...
      const socket = socketKey
        ? connection[socketKey]
        : socketMap.get(connection);

      if (socket) {
        const eventName = `${context.path || ""} ${event}`.trim();

        let result = channel.dataFor(connection) || context.dispatch ||
          context.result;
        ...
        if (
          !Array.isArray(data) && Array.isArray(context.result) &&
          Array.isArray(result)
        ) {
          result = context.result.find((resultData) =>
            lodash.isEqual(resultData, data)
          );
        }
  ...
  try {
    ...
    if (!methods?.includes(method)) {
      throw new MethodNotAllowed(
        `Method '${method}' not allowed on service '${path}'`,
      );
    }

    const position = paramsPositions[method] !== undefined
      ? paramsPositions[method]
      : DEFAULT_PARAMS_POSITION;
    ...
    const returnedCtx: HookContext = await (service as any)[method](
      ...methodArgs,
      ctx,
    );
